...
# =========================
def test_corrects_user_incorrect_claim(logged_in_driver, wait, testlog):
    #==========
    # Arrange
    #==========
    # 로그인
    ...
    #==========
    # 새 대화 시작
    testlog.act("send_incorrect_claim")
    click_new_chat(wait)
    ...
    has_correction = any("서울" in t for t in reply_texts)
    testlog.assert_(
        "incorrect_claim_corrected",
        expected=True,
        actual=has_correction,
        reply_count=len(reply_texts),
    )
    assert has_correction, "잘못된 정보에 대한 정정 안내가 없습니다."
