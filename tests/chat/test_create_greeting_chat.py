...
# =========================
def test_new_chat_receives_expected_greeting_reply(logged_in_driver, wait, question, expected_keywords, testlog):
    #==========
    # Arrange
    #==========
    # 로그인
    _ = logged_in_driver
    testlog.arrange("logged_in_driver_ready", question=question, expected_keywords=expected_keywords)

    #==========
    # Act
    #==========
    # 새 대화 시작
    testlog.act("send_greeting_question")
    click_new_chat(wait)
    ...
    testlog.assert_(
        "greeting_reply_contains_expected_keyword",
        expected=True,
        actual=has_expected_reply,
    )
    assert has_expected_reply, f"질문 '{question}'에 대한 응답이 기대 키워드 {expected_keywords}를 포함하지 않습니다."
